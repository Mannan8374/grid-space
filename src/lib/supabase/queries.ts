...
export const getSharedWorkspaces = async (userId: string) => {
  ...
  const sharedWorkspaces = (await db
    .selectDistinct({
      ...
      title: workspaces.title,
      iconId: workspaces.icon_id,
      logo: workspaces.logo, // ✅ FIX: include logo so shared workspaces also have it
    })
    .from(workspaces)
...
export const deleteFolder = async (folderId: string) => {
  ...
  // ✅ FIX: delete all files inside this folder, not a file whose id == folderId
  await db.delete(files).where(eq(files.folder_id, folderId));
