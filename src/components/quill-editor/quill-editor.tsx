...
const QuillEditor: React.FC<QuillEditorProps> = ({
  ...
  // Resolve current entity details (file or folder)
  ...
  // Files inside this folder (for Option B folder view)
  const folderFiles = useMemo(() => {
    if (dirType !== "folder" || !workspaceId || !fileId) return [];
    const workspace = state.workspaces.find((ws) => ws.id === workspaceId);
    const folder = workspace?.folders.find((f) => f.id === fileId);
    return folder?.files || [];
  }, [dirType, state, workspaceId, fileId]);
  ...
  // Initialize Quill ONLY for files
  const initializeQuill = async (wrapper: any) => {
    if (dirType !== "file") return;
  ...
  const wrapperRefCallback = useCallback(
    ...
    // dirType intentionally not included to avoid re-init on prop change
    []
  );
  ...
  // Load initial content for FILES only
  useEffect(() => {
    if (!fileId || dirType !== "file") return;
  ...
  // Handle other users' cursor movement (FILES only)
  useEffect(() => {
    if (dirType !== "file") return;
    ...
    const socketHandler = (range: any, roomId: string, cursorId: string) => {
      ...
      cursorModule.moveCursor(cursorId, range);
    };
    console.log("socket on - cursor move");
  ...
  // Rooms (still only matters for files, because quill is null otherwise)
  useEffect(() => {
  ...
  // Send local text & cursor changes to others (FILES only)
  useEffect(() => {
    if (dirType !== "file") return;
    ...
    const selectionHandler = selectionChangeHandler(user.id);
    ...
    const quillHandler = (delta: any, _oldDelta: any, source: any) => {
      ...
      saveTimerRef.current = setTimeout(async () => {
        if (contents && quillLength !== 1 && fileId) {
          // FILE save only (no folder text content now)
          if (!workspaceId || !folderId) return;
          dispatch({
            type: "UPDATE_FILE",
            payload: {
  ...
  // Apply remote text changes (FILES only)
  useEffect(() => {
    if (dirType !== "file") return;
    ...
    console.log("getting changes socket on");
    return () => {
  ...
  // Presence & collaborator avatars + cursor creation (FILES only)
  useEffect(() => {
    console.log("channel");
    if (dirType !== "file") return;
  ...
  }, [fileId, quill, supabase, user, dirType]);
    ...
    <>
      ...
      >
        ...
        >
          ...
          {/* Folder info – Option B (only for folders) */}
          {dirType === "folder" && (
            <div className="mt-4">
              <h2 className="text-base font-semibold mb-2">Files</h2>
              {folderFiles.length === 0 ? (
                ...
                <ul className="space-y-1">
                  {folderFiles.map((file) => (
                    <li key={file.id}>
                      <a
                        href={`/dashboard/${workspaceId}/${fileId}/${file.id}`}
                        className="flex items-center gap-2 text-sm text-card-foreground hover:underline"
                      >
                        <span className="text-lg">{file.icon_id}</span>
        ...
        {/* Only render the editor for files */}
        {dirType === "file" && (
          <div
            id="container"
      ...
      {/* Only show GenerateTemplate for files */}
      {dirType === "file" && (
